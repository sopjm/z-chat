...
#loading{position:fixed;inset:0;background:#000;z-index:9999;display:flex;flex-direction:column;justify-content:center;align-items:center;gap:15px;transition:.3s}
...
.input-area{display:flex;gap:6px;padding:10px;background:#181818;border-top:1px solid #444}
...
.file-btn{background:#555!important;color:white!important}
...
<div class="app">
  ...
  <div id="settingsPage" class="page">
    <div class="card">
      ...
      <div class="small-note">알림 허용 후 앱이 꺼져 있어도 Web Push 알림을 받을 수 있습니다.</div>
    </div>
  </div>
...
function saveRoom(code){
  ...
  if(!savedRooms.includes(code))savedRooms.unshift(code);
...
function renderGuestSavedRooms(){
  ...
  savedRooms.forEach(function(code){
    box.innerHTML+=
      ...
      "<div class='room-item-sub'>코드/이름 없이 바로 입장</div>"+
      "</div>";
  });
}
...
async function loadAdminStats(){
  ...
  document.getElementById("totalUsersText").innerText=
    "전체 접속자: "+data.totalUsers+"명 / 총 가입자: "+data.registeredUsers+"명";
...
async function leaveRoom(){
  try{
    await fetch("/leave",{
      method:"POST",
      headers:{"Content-Type":"application/json"},
      body:JSON.stringify({userId:userId,roomCode:roomCode})
    });
  }catch(e){}
...
async function loadMessages(){
  ...
  var res=await fetch("/messages?room="+encodeURIComponent(roomCode));
  var data=await res.json();
  ...
  data.forEach(function(m){
    ...
    var msgId=String(m.time)+"_"+String(m.userId)+"_"+String(m.text||m.fileName||"");
...
async function sendText(){
  ...
  await fetch("/send",{
    method:"POST",
    headers:{"Content-Type":"application/json"},
    body:JSON.stringify({userId:userId,roomCode:roomCode,name:userName,type:"text",text:text})
  });
...
function savePushUser(userId,roomCode,subscription){
  ...
  if(!pushUsers[userId])pushUsers[userId]={subscription:subscription,rooms:{}};
  ...
  for(var id in pushUsers){
    ...
    if(item.rooms&&item.rooms[roomCode]&&item.subscription){
      try{
        ...
        if(e.statusCode===404||e.statusCode===410)delete pushUsers[id];
      }
    }
  }
}
...
const server=http.createServer(function(req,res){
  ...
  if(path==="/")sendHtml(res,false);
    ...
    readBody(req,function(body){
      var data=JSON.parse(body);
      var code=String(data.roomCode||"").trim();
      if(!code){
        res.writeHead(200,{"Content-Type":"application/json"});
    ...
    var roomCode=url.searchParams.get("room");
    res.writeHead(200,{"Content-Type":"application/json"});
    ...
    for(var code in rooms){
      ...
      roomList.push({code:code,users:room.users.size,messages:room.messages.length});
    }
    res.writeHead(200,{"Content-Type":"application/json"});
    res.end(JSON.stringify({totalUsers:totalUsers,registeredUsers:registeredUsers.size,rooms:roomList}));
  }
    ...
    readBody(req,function(body){
      ...
      room.users.set(data.userId,{name:data.userName||"익명",time:Date.now()});
      registeredUsers.add(data.userId);

      if(pushUsers[data.userId]&&data.roomCode)pushUsers[data.userId].rooms[data.roomCode]=true;
    ...
    var msgUrl=new URL(req.url,"http://localhost");
    var msgRoomCode=msgUrl.searchParams.get("room");
    var msgRoom=getRoom(msgRoomCode);
    res.writeHead(200,{"Content-Type":"application/json; charset=utf-8"});
    res.end(JSON.stringify(msgRoom.messages));
  }
    ...
    readBody(req,function(body){
      ...
      var bodyText=msg.type==="image"?"사진을 보냈습니다.":msg.type==="video"?"영상을 보냈습니다.":msg.text||"새 메시지";
