...
.lightning { font-size: 80px; animation: flash 0.8s infinite alternate; }
...
.name { font-size: 12px; margin-bottom: 5px; opacity: 0.7; font-weight: bold; }
...
function renderGuestSavedRooms() {
  const box = document.getElementById("guestSavedRooms");
  if (!box) return;
...
async function createRoom() {
  ...
  roomCode = code;
  await fetch("/create-room", { method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({roomCode}) });
  saveRoom(code);
  startChat();
}
...
async function leaveRoom() {
  ...
  try {
    await fetch("/leave", {
      ...
      body: JSON.stringify({userId, roomCode: leavingRoom})
    });
  } catch (e) {}
...
async function enter() {
  if (!roomCode) return;
  const res = await fetch("/enter", { method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({userId, roomCode, userName}) });
  const data = await res.json();
  if (!data.ok) {
...
async function sendText() {
  ...
  if (!text.trim()) return;
  await fetch("/send", { method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({userId, roomCode, name: userName, type: "text", text}) });
...
function sendFile() {
  ...
  if (!file) return;
  if (file.size > ${MAX_UPLOAD_SIZE}) return alert("5MB 이하만 가능");
  if (!file.type.startsWith("image/") && !file.type.startsWith("video/")) return alert("사진이나 영상만 가능해");
  const reader = new FileReader();
  reader.onload = async () => {
    await fetch("/send", { method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({ userId, roomCode, name: userName, type: file.type.startsWith("image/") ? "image" : "video", data: reader.result, fileName: file.name }) });
...
const server = http.createServer((req, res) => {
  if (req.url === "/") sendHtml(res, false);
  else if (req.url === "/z-admin" || req.url === "/admin") sendHtml(res, true);
    ...
    for (const code in rooms) {
      ...
      roomList.push({ code, users: room.users.size, messages: room.messages.length });
    }
    res.writeHead(200, {"Content-Type": "application/json"});
    res.end(JSON.stringify({ totalUsers, rooms: roomList }));
  }
