...
const server = http.createServer(function(req, res) {
  if (req.url === "/") sendHtml(res, false);
  ...
  else if (req.url === "/manifest.json") { res.writeHead(200, {"Content-Type":"application/json"}); res.end(manifestUser); }
  else if (req.url === "/admin-manifest.json") { res.writeHead(200, {"Content-Type":"application/json"}); res.end(manifestAdmin); }
  else if (req.url === "/service-worker.js") { res.writeHead(200, {"Content-Type":"application/javascript"}); res.end(serviceWorker); }
  else if (req.url === "/icon.svg") { res.writeHead(200, {"Content-Type":"image/svg+xml"}); res.end(userIconSvg); }
  else if (req.url === "/admin-icon.svg") { res.writeHead(200, {"Content-Type":"image/svg+xml"}); res.end(adminIconSvg); }
  ...
  else if (req.url.startsWith("/room-users")) {
    ...
    let totalUsers = 0; const roomList = []; for (const code in rooms) { const room = rooms[code]; cleanUsers(room); totalUsers += room.users.size; roomList.push({ code, users: room.users.size, messages: room.messages.length }); } res.writeHead(200, {"Content-Type":"application/json"}); res.end(JSON.stringify({ totalUsers, rooms: roomList }));
  }
  else if (req.url === "/enter" && req.method === "POST") {
    readBody(req, body => { const data = JSON.parse(body); const room = getRoom(data.roomCode); cleanUsers(room); if (!room.users.has(data.userId) && room.users.size >= MAX_USERS_PER_ROOM) { res.writeHead(200, {"Content-Type":"application/json"}); res.end(JSON.stringify({ok:false,count:room.users.size})); return; } room.users.set(data.userId, { name: data.userName || "익명", time: Date.now() }); if (pushUsers[data.userId] && data.roomCode) pushUsers[data.userId].rooms[data.roomCode] = true; res.writeHead(200, {"Content-Type":"application/json"}); res.end(JSON.stringify({ok:true,count:room.users.size})); });
  }
